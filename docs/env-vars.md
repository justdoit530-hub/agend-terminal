...
| `AGEND_BOT_TOKEN` | **Supported compatibility** Telegram bot token, read only when the configured `bot_token_env` var is unset; emits a notice recommending `bot_token_env`. | Both unset → "bot token env not set" error; telegram verify test is skipped. | Telegram bot token string. | `src/channel/telegram/creds.rs:25`; `src/channel/telegram/bootstrap.rs:39` | 🔒 Secret. **Supported compatibility** — read-time fallback only. `quickstart` now writes the canonical `AGEND_TELEGRAM_BOT_TOKEN` (and migrates a legacy line out on re-run); prefer `bot_token_env` in fleet.yaml. |
...
| `AGEND_TURN_SENTINEL_SHADOW` | #1523 Phase-0 **measure-only** shadow telemetry. When on, every backend (incl. Claude since #2366) gets an in-band "turn-completion sentinel" directive injected into its instructions — the agent prints a per-agent token as the last line of a fully-finished turn — and the daemon side-logs whether that token appears to `turn_sentinel_shadow.jsonl`. | Off (no directive injected, no telemetry). | `"1"` enables; any other value / unset → off. | `src/state/mod.rs:562` (`turn_sentinel_shadow_enabled`); directive at `src/instructions.rs:529`; detector `src/state/mod.rs:2464` (`capture_turn_sentinel_shadow`) | **Measure-only**: drives NO state or behaviour — Phase 0 takes no action on the signal, Claude's authoritative state still comes from its lifecycle hooks, and the shadow detector is fail-open. Default-off → zero behaviour change. Currently set during the operator's Phase-0 soak. |
| `AGEND_RECOVERY_SHADOW` | #1523 Phase-0 **measure-only** shadow telemetry (sibling of `AGEND_TURN_SENTINEL_SHADOW`). When on, the supervisor records a per-agent "would the recovery discriminator have fired?" verdict (recovery-turn expectation vs observed recovery) to `recovery_shadow.jsonl`. | Off (every entry point early-returns → byte-identical behaviour). | `"1"` enables; any other value / unset → off. | `src/daemon/recovery_shadow.rs:54` (`enabled`) | **Measure-only**: Phase 0 only RECORDS the verdict, NEVER acts on it. Default-off → zero behaviour change. |
