...
| `AGEND_TURN_SENTINEL_SHADOW` | #1523 Phase-0 **純測量（measure-only）** shadow 遙測。開啟時，每個 backend（含 Claude，#2366）的 instructions 會被注入一段 in-band「turn-completion sentinel」directive —— agent 在一個 turn 完全結束時於回覆最後一行印出 per-agent token；daemon 則 side-log 該 token 是否出現到 `turn_sentinel_shadow.jsonl`。 | 關閉（不注入 directive、不記錄遙測）。 | `"1"` 啟用；其他值 / unset → 關閉。 | `src/state/mod.rs:562`（`turn_sentinel_shadow_enabled`）；directive 注入於 `src/instructions.rs:529`；偵測器 `src/state/mod.rs:2464`（`capture_turn_sentinel_shadow`） | **純測量**：不驅動任何 state 或行為 —— Phase 0 對此訊號不採取任何動作，Claude 權威 state 仍來自其 lifecycle hooks，shadow 偵測器 fail-open。預設關閉 → 零行為改變。目前 operator 於 Phase-0 soak 中已設定。 |
| `AGEND_RECOVERY_SHADOW` | #1523 Phase-0 **純測量（measure-only）** shadow 遙測（`AGEND_TURN_SENTINEL_SHADOW` 的姊妹旗標）。開啟時，supervisor 會把每個 agent「recovery discriminator 是否會觸發」的判定（recovery-turn 預期 vs 實際 recovery）記錄到 `recovery_shadow.jsonl`。 | 關閉（每個進入點都 early-return → byte-identical 行為）。 | `"1"` 啟用；其他值 / unset → 關閉。 | `src/daemon/recovery_shadow.rs:54`（`enabled`） | **純測量**：Phase 0 只記錄判定，絕不據以行動。預設關閉 → 零行為改變。 |
