...
/// completes. With N=3 agents and a 2000 ms stagger, the legacy ordering
/// (agents-first) takes ~4 s before `api::serve` even starts; the budget here
/// is 3000 ms so the test fails on agents-first and still passes after C1 under
/// cold/loaded CI runners (the previous 1000/1900 pair flaked at ~2.02 s).
#[cfg(unix)]
#[test]
fn api_port_published_before_agent_spawn_loop_completes() {
    ...
    let mut child = Command::new(&bin)
        ...
        // deterministic ~4 s (N=3 × 2000 ms); the C1 reorder makes `api.port`
        // appear independently of this. Wider than the old 1000 ms so the
        // C1 budget can absorb cold-CI process/init noise without false fails.
    ...
    // Budget: well below agents-first wall time (~4 s with N=3 + 2000 ms stagger)
    // and above C1-reordered wall time under loaded local/CI runners (observed
    // cold macos CI at ~2.02 s under the old 1900 ms budget).
    ...
    assert!(
        ...
        "Bug 1: api.port MUST be published before the agent spawn loop completes. \
         With N=3 agents at 2000ms stagger, the legacy agents-first ordering \
         delays api.port by ~4s; this test budget is 3000ms. Elapsed: {elapsed:?}. \
