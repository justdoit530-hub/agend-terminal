...
#[test]
fn verified_report_without_category_records_success_and_corrects_general_mistake() {
    let _g = fleet_test_guard();
    let (_rec, home) = setup_recorder("fleet_report_verified_success_default_category");
    let mistakes_dir = home.join("mistakes");
    std::fs::create_dir_all(&mistakes_dir).expect("create mistakes dir");
    let mistake = crate::reflexion::Mistake {
        id: "mstk-general-review".to_string(),
        task_id: Some("T-success-default-category".to_string()),
        agent_name: "sender".to_string(),
        category: "general".to_string(),
        rejection_reason: "prior uncategorized review issue".to_string(),
        timestamp: chrono::Utc::now().to_rfc3339(),
        corrected_at: None,
    };
    std::fs::write(
        mistakes_dir.join("mstk-general-review.json"),
        serde_json::to_string(&mistake).expect("serialize mistake"),
    )
    .expect("write mistake");
    ...
    let result = handle_tool_rt(
        ...
        &json!({
            ...
            "request_kind": "report",
            "summary": "VERIFIED\n### Evidence\nran: cargo test -> passed",
            "correlation_id": "T-success-default-category",
        }),
        "sender",
    );
    assert!(
    ...
    assert!(
        successes_path.exists(),
        "VERIFIED report without category should record default-category success"
    );
    let successes: Vec<crate::reflexion::Success> =
        serde_json::from_str(&std::fs::read_to_string(successes_path).expect("read successes"))
            .expect("deserialize successes");
    ...
    let corrected: crate::reflexion::Mistake = serde_json::from_str(
        &std::fs::read_to_string(mistakes_dir.join("mstk-general-review.json"))
            .expect("read corrected mistake"),
    )
    .expect("deserialize corrected mistake");
    assert!(
        corrected.corrected_at.is_some(),
        "VERIFIED report without category should correct the sender's general mistake"
    );
...
fn test_send_to_inbox_fallback_mode() {
    // #2454 Slice 2: in-process SEND with runtime + fleet target not in the
    // live registry delivers via inbox_only (no socket loopback / no
    // silent inbox_fallback when runtime is absent).
    let _g = fleet_test_guard();
    ...
    assert_eq!(
        ...
        Some("inbox_only"),
        "fleet-known but not-running target delivers inbox_only: {result}"
    );
    // runtime=None path fails closed (no silent fallback).
    let no_rt = handle_tool(
        "send",
        &json!({"instance": "receiver", "message": "test"}),
        "sender",
    );
